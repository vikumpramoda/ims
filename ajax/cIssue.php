    ...
    if ($number >= 0) {
        for ($i = 0; $i < $number; $i++) {
            if (trim($_POST['item_id'][$i] != '') && trim($_POST['ex'][$i] != '')&& trim($_POST['reqID'][$i] != '')) {
                ...
                $sql = "INSERT INTO `issue_order`(`req_id`,`qty`,`depot_id`,`member_id`) VALUES('" . mysqli_real_escape_string($con, $_POST['reqID'][$i]) . "','" . mysqli_real_escape_string($con, $_POST['item_qty'][$i]) . "','$member_id','$depot')";
                mysqli_query($con, $sql);

                $query = "UPDATE req_items
                          SET `status` = 'issue' ,`issue_date` = CURDATE() , `issue_depot` = $depot  
                          WHERE reqID = ('" . mysqli_real_escape_string($con, $_POST['reqID'][$i]) . "')";
                $result = mysqli_query($con, $query);


                $r = "UPDATE main_stock
                        SET stock_level = stock_level - ('" . mysqli_real_escape_string($con, $_POST['item_qty'][$i]) . "')                
